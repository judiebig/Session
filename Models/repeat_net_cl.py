...
a simple version same in RE-GNN
'''
...
class RepeatNetCL(Module):
    def __init__(self, opt, n_node):
        super(RepeatNetCL, self).__init__()
    ...
    def forward(self, inputs, masks, re_masks, targets):
        ...
        sr = repeat_weight * repeat_sr + explore_weight * explore_sr
        cl_score = self.contrastive(sr, targets, cl_mask)
...
def train_and_test(model, train_data, test_data, opt):
    ...
    for epoch in range(opt.epoch):
        ...
        for i, j in zip(slices, tqdm(np.arange(len(slices)))):
            ...
            scores, cl_score = model(inputs, masks, re_masks, targets)
            ...
            train_loss += loss
            loss_cl_s.append(loss_cl)
            if j % int(len(slices) / 5 + 1) == 0:
                print('Loss: %.4f\t%.4f' % (loss.item(), opt.cl_lambda*loss_cl.item()))
                # if z==1:
                #     np.savetxt("repeat_loss_cl_ncl.txt", loss_cl_s)
                #     exit()
                # z+=1
